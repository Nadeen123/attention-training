const initialState = {
  counter: 1,
  score: localStorage.getItem("score") ? localStorage.getItem("score").split(",") : [],
  percent: 0,
  question: "",
...
function reducer(state = initialState, action) {
  switch (action.type) {
    ...
    case "INCREMENT_INSTRUCTIONS_COUNTER":
      return {
        ...state,
        instructionsCounter: (state.instructionsCounter % 4) + 1
      };
    case "DECREMENT_INSTRUCTIONS_COUNTER":
      return {
        ...state,
        instructionsCounter: state.instructionsCounter - 1
      };
    default:
      return state;
  }
